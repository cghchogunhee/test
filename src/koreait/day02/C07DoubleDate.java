...
/*  실수 데이터 기본 형식은  float, double 2가지 이며 
 * 				ㄴ 변수를 선언할 때 사용할 키워드(예약어. 용도가 정해진 단어)
 * 				ㄴ 각각 메모리에 할당받는 크기(바이트 단위)와 그에 따른 값의 표현 범위가 결정됩니다.
 *  
 * Float, Double 클래스는 실수 데이터 형식를 다루는 속성과 메소드를 갖는 클래스입니다.(Wrapper 래퍼 클래스)
 *		ㄴ BYTES, MIN_VALUE, MAX_VALUE :  위 클래스들의 구성요소인 특징(필드)들 중 변하지 않는값(상수) 입니다.
